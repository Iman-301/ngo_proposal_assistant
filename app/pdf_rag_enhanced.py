...
#         """Check if LLM backend is reachable to avoid repeated runtime errors."""
#         if not self.llm:
#             return False
#         try:
...
#     def load_pdfs(self, pdf_paths: List[str], append: bool = False) -> int:
#         """Load and chunk a list of PDF files."""
#         all_docs = self.documents[:] if append else []
#         loaded = 0
...
#     def load_pdfs_from_folder(self, folder_path: str, append: bool = False) -> int:
#         """Load all PDFs from a folder"""
#         pdf_files = glob.glob(os.path.join(folder_path, "*.pdf"))
#         return self.load_pdfs(pdf_files, append=append)
...
#         stop_words = {
#             "what", "is", "the", "for", "of", "to", "in", "a", "an", "and", "or",
#             "by", "does", "do", "required", "requirements", "are", "can", "how",
#             "who", "which", "when", "where", "why", "about", "please", "tell",
#         }
...
#     def create_retriever(self, k: int = 8, fetch_k: int | None = None):
...
#         if not self.llm or not self.llm_ready:
#             return ""
...
#             )
#         except Exception:
#             scored_docs = [(doc, None) for doc in self.vectorstore.max_marginal_relevance_search(
#                 question,
#                 k=min(self.k + 3, 12),
...
#         # Hybrid re-ranking: semantic + lexical match
#         ranked_docs: List[Tuple[Document, float]] = []
#         for doc, score in scored_docs:
#             lex = lexical_score(doc.page_content, self._extract_terms(question))
#             sem = score if isinstance(score, (float, int)) else 0.0
#             combined = (sem * 0.7) + (lex * 0.3)
#             ranked_docs.append((doc, combined))
#         ranked_docs.sort(key=lambda x: x[1], reverse=True)
#         docs = [doc for doc, _ in ranked_docs[:max(self.k, 8)]]
